...
class LocalFSVGRUpdate(LocalUpdate):
    def __init__(self, args, dataset, idxs, h, tb, bs):
        super(LocalFSVGRUpdate, self).__init__(args, dataset, idxs, h, tb, bs)
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                for i, param in enumerate(net.parameters()):
                    ...
                    if self.args.gpu != -1:
                        ...
                        param.data.sub_((self.h/(len(self.idxs)) * (self.lg_scalar * (client_w_grad[i] - server_w_grad[i]) +
